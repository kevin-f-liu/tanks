...
// Bitmap for blue tank, no barrel
...
// Init bitmap array for barrel
uint16_t barrelmap[TANK_WIDTH*TANK_WIDTH];

void printBarrelmap(void) {
  // Testing function to check barrel bitmap
  // Iterate over and print line by line
...
void initBarrelmap(char player) {
  // Start the barrel bitmap either pointing left or right
  loadBarrelmap(player == 1 ? 0 : 180);
}

void loadBarrelmap(int a) {
  // Generate a sprite map that is tank_width^2 size with a line of pixels with the angle specified
  // Begin line of black pixels from center and top of tank  
  ...
  int x, y, h2; // Store the square of the hypotenuse to avoid squarooting
  ...
  for (idx = 0; idx < TANK_WIDTH * TANK_WIDTH; idx++) {
    // Convert linear index to xy coordinate in barrel bitmap
    x = idx % TANK_WIDTH - startx;
    y = idx / TANK_WIDTH - starty;
    h2 = x*x+y*y;
    cursin = y*fastInvsqrt(h2); // Value of sin of current coordinate
    curcos = x*fastInvsqrt(h2); // Value of cos of current coordinate
    // Pixel is part of barrel if within tolerance, generates a segment of a circle
...
// Black square representing shot
...
// Slope going up to right bitmap
...
// Different exploded block bitmap
