...
uint32_t getIndex(uint16_t x, uint16_t y) {
  return TERRAIN_WIDTH * y + x;
}
Coordinate getCoord(uint16_t idx) {
  Coordinate c = {.x = idx % TERRAIN_WIDTH, .y = idx / TERRAIN_WIDTH};
...
void generateTerrain(Terrain *terrain) {
  ...
  for (int i = 0; i < TERRAIN_WIDTH; i++) {
    curRand = random(-1, 1);
    // if curRandom is the opposit of prevRand, smooth out the change
    ...
    if (row == 0)
      row = 1;
    // prevent out of range row being populated
...
// set t(x,y) to false if it's not out of range
